...
## 4. TUI Command & Overlay

The same computation is available from the TUI command prompt (`:`):

```text
:patent.expiration-date            # compute for the focused/selected patent
:patent.expiration-date refresh    # force a live USPTO re-fetch, then recompute
:patent.expiration-date US14558776 # compute for an explicit number
```

The command is registered as `patent.expiration-date` with the aliases `expiration-date`, `expiration`, and `exp`, so `:exp` works the same way. It is available in the catalog, detail, citations, and family scopes; when no number is typed it uses the focused pane's current selection.

Running the command opens the **Patent Expiration Analysis** overlay, which calls the daemon's `uspto.expiration.calculate` RPC and shows the USPTO inputs (filing/grant dates, earliest-term source application, PTA, PTE, terminal disclaimer), the computed expiration date, the Google Patents estimate, and the comparison line. Inside the overlay:

- **`r`** — refresh/recompute with a live USPTO fetch.
- **`Esc` / `q` / `Enter`** — close the overlay.

Wiring: `internal/tui/app_commands.go` (`cmdPatentExpirationDate` → `openExpirationOverlay`), the overlay in `internal/tui/overlay/expiration.go`, and the server handler in `internal/rpc/server.go` (`usptoExpirationCalculate`).

---

## 5. Stored Data Schema

The expiration-date computation stores its results in the `uspto_application` table in the local SQLite database (`patentmine.db`). The data is persisted via `store.NodeBatch` → `repo.SaveNode()` which calls the `INSERT … ON CONFLICT(application_number) DO UPDATE` upsert in `internal/store/sqlite/source_data.go`. The five expiration columns are added to existing databases by the idempotent `ensureExpirationColumns` migration in `internal/store/sqlite/sqlite.go`.

> [!NOTE]
> The upsert writes the expiration columns from `excluded` on every save, so an ordinary re-crawl of the same record (which carries no computed expiration) resets these columns until `expiration-date` is run again. The computation is idempotent, so re-running restores them.
...
| `record_number` | TEXT | patent table FK | Links to `patent.number` (the canonical record) |
...
| `parent_record_number` | TEXT | Canonical record number for the parent (links to `patent.number`) |
| `child_record_number` | TEXT | Canonical record number for the child (links to `patent.number`) |
