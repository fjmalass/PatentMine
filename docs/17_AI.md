...
To minimize heavy package import overhead, PatentMine implements a custom environment variables resolver inside [config.go](../internal/config/config.go).
...
All outbound requests in the Gemini client go through a secure `RedactedRoundTripper` (implemented inside [observability.go](../internal/ai/observability.go)). This custom `http.RoundTripper` intercepts outbound payloads and:
