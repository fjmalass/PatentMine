...
var cmdStrings = map[string][2]string{
	...
	"view.assignees":                         {"Open assignee timeline", "Open the single-patent ownership timeline showing recorded USPTO assignment details."},
	"view.assignees-project":                 {"Project Assignees Rollup", "Open the active project's assignee rollup: current owners (live patents only) vs. every assignee ever, with live/expired/not-fetched coverage."},
	"view.all-assignees-history":             {"Open all assignees history", "Open the history popup for all assignees, preselecting the current patent's assignee when available."},
	...
	"view.browser-uspto":                     {"Browse USPTO", "Open the USPTO grant XML URL, falling back to the publication XML URL."},
	"view.browser-uspto-grant":               {"Browse USPTO grant", "Open the selected patent's USPTO grant XML URL."},
	"view.browser-uspto-pgpub":               {"Browse USPTO publication", "Open the selected patent's USPTO pre-grant publication XML URL."},
	"view.browser-google":                    {"Browse Google", "Open the selected patent on Google Patents, regardless of saved source."},
	"view.metrics":                           {"Metrics", "Open the daemon metrics dashboard overlay."},
	"docs":                                   {"Docs", "Open the project documentation list from the daemon."},
	"docs.open":                              {"Open docs", "Open the project documentation list from the daemon."},
	"docs.show":                              {"Show doc", "Open one project documentation file by ID, such as README.md or docs/00_HOW_TO_TUI.md."},
	...
	"patent.fetch-uspto":                     {"Fetch USPTO XML", "Download the best available USPTO XML for the selected patent — the grant when one is on record, otherwise the pre-grant publication."},
	...
	"patent.fetch-uspto-assignments-project": {"Fetch Assignments (project)", "Batch-fetch recorded assignments for every curated member of the active project; expired patents are skipped unless 'include-expired' is given."},
	...
	"officeaction.assign-patents":            {"Assign to office action", "Assign the selected patent(s) to office action(s) for review. Like :tag.patent.add <name>, pass an office-action name to assign directly (matches its label, case-insensitive); with no argument an office-action picker opens. The assignment shows in the catalog OA column and is filterable with oa:."},
	"officeaction.release-patents":           {"Remove from office action", "Remove the selected patent(s) from office action(s). Pass an office-action name to act directly, or run with no argument to open the picker."},
	...
	"conflict.flag":                          {"Flag conflict", "Flag a record as conflicting within the active matter — material prior art, an interfering application, a reference reading on the claims. Pass the patent number and an optional reason, e.g. :conflict.flag US10000000B2 anticipates claim 1. Conflicting records show a ⚠ badge in the patent list."},
	...
	"deadline.track":                         {"Track renewals", "Track a granted patent's U.S. maintenance-fee deadlines, derived from its grant date (due at 3.5 / 7.5 / 11.5 years). Pass the patent number and optionally the entity size (large, small, micro), e.g. :renewal.track US10000000B2 small."},
	"deadline.untrack":                       {"Untrack renewals", "Stop tracking a patent's U.S. maintenance-fee/annuity deadlines. This will delete any pending maintenance deadlines and set the patent as untracked. Pass the patent number, e.g. :renewal.untrack US10000000B2."},
	"renewal.validation.fetch":               {"Fetch renewal validations", "Pull EPO OPS legal-status data for an EP patent and derive post-grant national validation country states."},
	"renewal.validation.list":                {"Show renewal validations", "Show stored country-phase validation states for a patent."},
	"renewal.validation.set":                 {"Set renewal validation", "Manually set one country-phase state: potential, validated, lapsed, or unknown."},
	...
	"tag.patent.add":                         {"Assign patent tag", "Assign an existing taxonomy tag to the selected patent; unlike `tag`, the tag must already exist."},
