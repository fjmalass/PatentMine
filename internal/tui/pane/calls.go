...
func CrawlCmd(client *rpc.Client, number domain.PatentNumber, depth int, profile domain.CrawlProfile, force bool, source domain.Source) tea.Cmd {
	return func() tea.Msg {
		ctx, cancel := callContext()
		defer cancel()
...
func MultiCrawlCmd(client *rpc.Client, numbers []domain.PatentNumber, depth int, profile domain.CrawlProfile, force bool, source domain.Source) tea.Cmd {
	return func() tea.Msg {
		...
		for _, n := range numbers {
			go func(n domain.PatentNumber) {
				...
				var res proto.CrawlStartResult
				err := client.Call(ctx, proto.MethodCrawlFamily,
					proto.CrawlFamilyParams{Root: n, Depth: depth, Profile: profile, Force: force, Source: source}, &res)
				if err != nil {
