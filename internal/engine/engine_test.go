...
func TestEngineAddToProjectCreatesStubForUnknownPatent(t *testing.T) {
	...
	if _, _, _, err := eng.AddToProject(ctx, project.ID, number); err != nil {
...
func TestEngineAddToProjectAutoFetchesNewStub(t *testing.T) {
	...
	if _, _, _, err := eng.AddToProject(ctx, project.ID, domain.MustParsePatentNumber("AU607081B2")); err != nil {
...
func TestEngineResolvesDocumentNumberToRecord(t *testing.T) {
	...
	if _, _, _, err := eng.AddToProject(ctx, project.ID, application); err != nil {
