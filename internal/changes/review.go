...
// citationReview sets the review state of one or more citation edges and, in
// the same change, the review state of each edge's target patent so the patent
// list column stays in sync.
//
...
// SetCitationReviewState builds a change that moves every edge to next, and
// moves each edge's target patent to next as well so the patent list column
// stays in sync. A target not yet in the project produces no patent row update.
func SetCitationReviewState(projectID domain.ProjectID, edges []domain.CitationEdge, next string) Change {
...
func SetPatentReviewState(projectID domain.ProjectID, numbers []domain.PatentNumber, next string) Change {
