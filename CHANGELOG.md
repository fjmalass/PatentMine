...
- **TUI**: `:add.officeaction` now opens a **metadata form** (examiner, mail date, type, art unit, application number — pre-filled from the project) before import, so the examiner name is captured. New `:add.document` files a supporting document, `:open.documents` lists the matter's documents (enter views the text in a read-only vim viewer, `r` renames, `d` deletes), and `:set.matter <type>` records the prosecution stage.
