...
- **TUI**: `:add.officeaction [path]` (verb-first) imports an Office Action from any directory into the active project. With no argument it opens a **file picker** (`bubbles/filepicker` modal overlay, `.pdf`/`.txt`); the file is copied into the docs export store. The TUI/daemon share a filesystem (unix socket), so the picker browses locally and only the path crosses RPC.
